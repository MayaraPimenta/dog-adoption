  ...
  <section class="bg-secondary-green py-12 tablet:px-24">
    <div class="max-size">
      ...
      <p class="text-center mt-4">
        Entre em contato conosco tire dúvidas, saiba como apoiar o projeto e mais.
      </p>
      ...
      <div class="flex flex-col gap-4 tablet:gap-6 tablet:flex-row mt-6">
        ...
        <div class="flex flex-col w-full">
          ...
          <div
            v-if="errors?.name"
            class="text-red-500 text-xs text-xs"
          >
            <span
              v-for="(error, index) in errors.name._errors"
              :key="index"
            >{{ error }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-col w-full">
          ...
          >
            <span
              v-for="(error, index) in errors.phone._errors"
              :key="index"
            >{{ error }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-col w-full">
          ...
          >
            <span
              v-for="(error, index) in errors.email._errors"
              :key="index"
            >{{ error }}</span>
          </div>
        </div>
      ...
      >
        <span
          v-for="(error, index) in errors.comments._errors"
          :key="index"
        >{{ error }}</span>
      </div>
